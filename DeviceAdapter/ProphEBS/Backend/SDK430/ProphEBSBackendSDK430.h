...
//                Metavision SDK 4.3.0's "driver" module
//                (metavision_sdk_driver), deliberately NOT the "stream"
//                module even though this repo's vendored SDKs/430/ devkit
//                happens to ship both -- a real-world 4.3.0 field install
//                (the incident that originally motivated the backend-shim
//                architecture; see docs/DEVLOG.md) only shipped
//                metavision_sdk_driver.dll, not metavision_sdk_stream.dll,
//                so building against "driver" is what actually makes this
//                backend loadable on that class of machine. The "driver"
//                module's Camera class lacks the "stream" module's
//                convenience Camera::get_facility<T>() (throwing, T&) --
//                see Facility<T>() below, which normalizes onto the same
//                shape via the lower-level, pointer-returning
//                Camera::get_device().get_facility<T>() instead.
//                Ships as ProphEBS_Backend_SDK430.dll, loaded dynamically at
//                runtime by mmgr_dal_ProphEBS.dll via BackendLoader -- see
...
class ProphEBSBackendSDK430 : public IProphEBSBackend
{
public:
   ProphEBSBackendSDK430() = default;
   ~ProphEBSBackendSDK430() override;
...
private:
   // The "driver" module's Camera has no get_facility<T>() convenience --
   // only the lower-level Camera::get_device().get_facility<T>(), which
   // returns a possibly-null T* instead of a throwing T&. Every call site in
   // this backend is written as a try/catch around Facility<T>(), so
   // normalizing onto the same throwing-T&-reference shape here (matching
   // ProphEBSBackendSDK511's identical-looking helper) keeps the rest of
   // this file byte-for-byte the same between backends.
   template <typename T>
   T& Facility()
   {
